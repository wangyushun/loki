...
func (b *IHarborObjectClient) List(ctx context.Context, prefix string, delimiter string) ([]chunk.StorageObject, []chunk.StorageCommonPrefix, error) {
	...
	for {
		...
		for _, object := range results.Contents {
			if object.IsObject {
				...
				commonPrefixes = append(commonPrefixes, chunk.StorageCommonPrefix(commonPrefix))
			}
		}
...
func (b *IHarborObjectClient) Attributes(ctx context.Context, objectKey string) (objstore.ObjectAttributes, error) {
